...
class GDMLTrain(object):
    ...
    def inverse(self,task,model,initial=0,c = 1):
        
        
        sig_optim = model['sig_optim']
        alphas_opt = model['alphas_opt']
        sigma_2_hat = model['sigma_2_hat']
        ...
        np.random.seed(10)
        R_val = task['R_train'][initial,:,:] + (np.random.normal(size = n_atoms*3)*1e-2).reshape(-1,3)
