...
Record=AFF_train.inverseE_new( task,trained_model,E_target,ind_initial=initial,tol_MAE=0.01,lr=1e-1,c=2,num_step = 20,random_val = 1e-2)
...
while n_loop<20:
    ...
    candid_range = np.exp(np.arange(-1,0,1))
    #task['lam'] = 1e-10
    ...
    Record=AFF_train.inverseE_new( task,trained_model,E_target,ind_initial=initial,tol_MAE=0.01,lr=1e-1,c=2,num_step = 30,random_val = 1e-2)
