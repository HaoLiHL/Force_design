...
#task_test{}.npy
# task1=AFF_train.create_task(dataset_sub,n_train,dataset_sub,200,100,10,1e-5)
...
# np.save('./dataset/task_asp_800.npy', task1) 
# np.save('./dataset/trained_model_asp_800.npy', trained_model) 

# sigma_opt=0.36787944117144233
...
#task=np.load('dataset/task_asp_800.npy',allow_pickle=True).item()
#trained_model = np.load('dataset/trained_model_asp_800.npy',allow_pickle=True).item()
...
Record=AFF_train.inverseE_new( task,trained_model,E_target,ind_initial=initial,tol_MAE=1e-2,lr=1e-2,c=0.1,num_step = 10,random_val =1e-3)

#Record=AFF_train.inverseE_new( task,trained_model,E_target,ind_initial=initial,tol_MAE=1e-5,lr=1e-2,c=0.01,num_step = 10,random_val =1e-3)
...
while n_loop<10:
    ...
    candid_range = np.exp(np.arange(-2,3,1))
    task['lam'] = 1e-10
    ...
    if new_E[0]<=task['E_train'][initial]:
        initial=n_train
    #Record=AFF_train.inverseE_new( task,trained_model,E_target,ind_initial=initial,tol_MAE=0.01,lr=1e-3,c=0.01,num_step = 15)
       
    Record=AFF_train.inverseE_new( task,trained_model,E_target,ind_initial=initial,tol_MAE=1e-3,lr=1e-2,c=0.1,num_step = 30,random_val = 1e-3)
