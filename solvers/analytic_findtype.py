...
# your code here    
        ...
        for ind_i in range(n_type):  # get the prediction for all C and all H atoms
            ...
            if task['use_E_cstr']:
                # the index of each groups
                #np.append(index_eg,3*n_atoms)
                #index_x=np.repeat(np.arange(n_val)*(dim_i+1),3*len(index_diff_atom[ind_i])+1)+np.tile(np.append(index_eg,3*n_atoms),n_val)
                index=np.repeat(np.arange(n_train)*(dim_i+1),3*len(index_diff_atom[ind_i])+1)+np.tile(np.append(index_eg,3*n_atoms),n_train)

                #index_y=np.repeat(np.arange(n_train)*(dim_i+1),4)+np.tile(np.array([3*ind_i,3*ind_i+1,3*ind_i+2,3*n_atoms]),n_train)
            else:
