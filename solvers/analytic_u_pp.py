...
# analytic_u_ for parallel computation
"""
...
class Analytic(object):
    ...
    def solve_xyz(self, task,sig_candid, R_desc, R_d_desc, tril_perms_lin, tril_perms_lin_mirror, y):
        ...
        alphas = mp.RawArray('d',  3*n_train * n_type)
        
        glob['alphas'], glob['alphas_shape'] = alphas, (3*n_train , n_type)
...
# your code here    
        ...
        alphas=np.frombuffer(alphas).reshape(glob['alphas_shape'])
        stop = timeit.default_timer()
...
def solve_pp(ind_i,K_all,lam,n_train,dim_i,y):
    global glob
    ...
    K=K_all[np.ix_(index,index)]
       ...
       # a=K_r_allr[index,:]
       # K=a[:,index]
    alphas = np.frombuffer(glob['alphas']).reshape(glob['alphas_shape'])
