...
# candid_range = np.arange(10,50,10)
# trained_model= AFF_train.train(task,candid_range,np.arange(0.1,1,0.1))
# # trained_model['S2_optim']
# np.save('saved_model/task_asp400.npy', task) 
# np.save('saved_model/trained_model_asp400.npy', trained_model) 


task=np.load('saved_model/task_asp400.npy',allow_pickle=True).item()
trained_model=np.load('saved_model/trained_model_asp400.npy',allow_pickle=True).item()
...
R_proposed_tensor,F_predict = AFF_train.inverse(task,trained_model,initial=0, c = 1e-10, n_iter = 100,random_noise = 1e-4)   
